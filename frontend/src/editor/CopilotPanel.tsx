...
export const CopilotPanel = ({ graphId }: CopilotPanelProps) => {
  ...
  const isPending =
    initiateCopilot.isPending || approvePlan.isPending || applyPatch.isPending;
  ...
  const handleApprovePlan = async (approved: boolean) => {
    try {
      ...
      setCopilotState(res);
      if (!approved) {
        // Reset local input if cancelled
        setPrompt('');
      }
    } catch (err) {
      console.error(err);
  ...
  const showPlanDialog = copilotState?.status === 'pending_plan_approval';
  const showApplyDialog = copilotState?.status === 'pending_apply_approval';
  ...
  return (
    <>
      ...
      <Dialog.Root open={showPlanDialog} onOpenChange={() => handleApprovePlan(false)}>
        <Dialog.Content style={{ maxWidth: 500, backgroundColor: '#202024', color: 'white' }}>
          ...
          <Flex gap="3" justify="end">
            <Dialog.Close>
              <Button variant="soft" color="gray" onClick={() => handleApprovePlan(false)}>
                Cancel
              </Button>
            </Dialog.Close>
            <Button onClick={() => handleApprovePlan(true)} disabled={isPending}>
      ...
      <Dialog.Root open={showApplyDialog} onOpenChange={() => handleApplyPatch(false)}>
        <Dialog.Content style={{ maxWidth: 550, backgroundColor: '#202024', color: 'white' }}>
          ...
          <Flex gap="3" justify="end">
            <Dialog.Close>
              <Button variant="soft" color="gray" onClick={() => handleApplyPatch(false)}>
                Reject
              </Button>
            </Dialog.Close>
            <Button
