...
def upsert_logical_assigner(flow_data: GraphFlowData, op: UpsertLogicalAssignerOp) -> GraphFlowData:
    ...
    from app.graphs.nodes.logical_assigner import LogicalAssignerNode, LogicalAssignmentSchema
...
def upsert_agentic_assigner(flow_data: GraphFlowData, op: UpsertAgenticAssignerOp) -> GraphFlowData:
    from app.graphs.nodes.agentic_assigner import AgenticAssignerNode
...
def upsert_rag_retriever(flow_data: GraphFlowData, op: UpsertRagRetrieverOp) -> GraphFlowData:
    from app.graphs.nodes.rag_retriever import RagRetrieverNode
...
def upsert_logical_switch(flow_data: GraphFlowData, op: UpsertLogicalSwitchOp) -> GraphFlowData:
    from app.graphs.expressions import get_expression_variables
    from app.graphs.expressions.translator import translate_polars_to_python
    from app.graphs.nodes.logical_switch import Branch, LogicalSwitchNode, _make_slot_id
...
def upsert_agentic_switch(flow_data: GraphFlowData, op: UpsertAgenticSwitchOp) -> GraphFlowData:
    from app.graphs.nodes.agentic_switch import AgenticBranch, AgenticSwitchNode, _make_slot_id
...
def upsert_interrupt(flow_data: GraphFlowData, op: UpsertInterruptOp) -> GraphFlowData:
    from app.graphs.nodes.interrupt import InterruptNode
